...
export default function Navbar({ onNavigate }) {
  ...
  const links =
    role === "admin" || role === "manager"
      ? [...NAV_LINKS.slice(0, 7), teamLink, ...NAV_LINKS.slice(7)]
      : NAV_LINKS;
    ...
    const fetchBadge = async () => {
      try {
        const [notifRes, docRes] = await Promise.all([
          ...
          api.get("/admin/document-alerts/count"),
        ]);
        const unread = (notifRes.data || [])
          .filter((n) => !n.read)
          .reduce((sum, n) => sum + (n.count || 1), 0);
        setNotificationCount(unread);
        setDocumentAlertCount(Number(docRes?.data?.count || 0));
      } catch {
        ...
        {links.map((link) => {
          ...
          const badgeHint =
            link.badgeKey === "notifications" && showBadge
              ...
              : link.to === "/website-applications" && showBadge
                ? `${notifBadge} pending document alerts`
              : undefined;
