...
export default function LeadProfile() {
  ...
  useEffect(() => {
    ...
    const showSidebar = () => {
      if (cancelled) return;
      /* Admission panel + lead details live here — keep in sync with intel so staff never wait on a blank sidebar. */
      setSidebarIntelReady(true);
      setSidebarRestReady(true);
      setChatThreadReady(true);
    };
    let idle1;
    if (typeof window.requestIdleCallback === "function") {
      idle1 = window.requestIdleCallback(showSidebar, { timeout: 120 });
    } else {
      idle1 = setTimeout(showSidebar, 56);
    }
    return () => {
      cancelled = true;
      if (typeof window.requestIdleCallback === "function") {
