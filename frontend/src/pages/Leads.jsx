...
import { userIdFromToken, roleFromToken } from "../utils/jwt";
...
export default function Leads() {
  ...
  const canRemoveLeads = myRole && myRole !== "viewer";
  ...
  const removeSelected = useCallback(async () => {
    ...
    if (failed.length > 0) {
      alert(`${failed.length} lead${failed.length !== 1 ? "s" : ""} could not be removed.`);
    }
  }, [selectedIds]);
  ...
  const visibleIds = visibleLeads.map((l) => String(l._id));
  ...
  return (
    <div className="page-shell leads-page" style={{ maxWidth: 1100 }}>
      <header className="leads-hero">
        ...
        <div className="leads-hero__search">
          <label htmlFor="leads-search" className="sr-only">
            Search by name, phone, email, source, country, or course
          </label>
          <div className="leads-search-wrap">
            ...
            <input
              ...
              placeholder="Search name, phone, email, source, country…"
              value={search}
      ...
      <div className="leads-table-controls">
        ...
        {canRemoveLeads && selectedIds.size > 0 && (
          <button
            type="button"
      ...
      <div className="leads-table-wrap panel">
        <div className="leads-table-head" aria-hidden>
          <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {canRemoveLeads && visibleLeads.length > 0 && (
              <input
                type="checkbox"
        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            const isSelected = selectedIds.has(String(lead._id));
            const assignedSame =
              Boolean(
                myUserId &&
                  assignedRaw &&
            ...
            return (
              <div
                key={lead._id}
                className={`leads-table-row lead-row${isSelected ? " leads-row--selected" : ""}`}
                onClick={open}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                ...
                >
                  {canRemoveLeads && (
                    <input
                      ...
                      className="leads-checkbox"
                      checked={isSelected}
                      aria-label={`Select ${lead.name || lead.phone || "lead"}`}
                      onClick={(e) => e.stopPropagation()}
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    {isWebsiteLead(lead) ? (
                      <button
                        type="button"
                        className="btn btn-secondary leads-review-mobile"
                        style={{ marginTop: 7, fontSize: 11, padding: "4px 10px", fontWeight: 600 }}
                        onClick={(e) => { e.stopPropagation(); navigate(`/website-intake/${lead._id}`); }}
                      >
                        Review application
                      </button>
                    ) : null}
                ...
                <div className="leads-source" data-label="Source" style={{ fontSize: 13, fontWeight: 500, color: "var(--text-2)" }}>
                  ...
                  {isWebsiteLead(lead) ? (
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/website-intake/${lead._id}`);
                      }}
                    >
                      Review application
                    </button>
        ...
        >
          ...
          <div style={{
            ...
            <p style={{
              ...
              and all their chat history. There is no way to recover them after deletion.
            </p>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              >
                ...
                Yes, delete {selectedIds.size === 1 ? "lead" : `${selectedIds.size} leads`}
              </button>
            </div>
          </div>
        </div>
